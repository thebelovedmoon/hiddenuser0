...
function urlQue() {
  if (urlArgs === "terminated") {
    ...
    parent.document.title = "CONNECTION TERMINATED";
    if (window.self !== window.top) {
      parent.document.querySelector('#dlg').onclick = () => { document.querySelector('#dlg').remove(); }
      parent.document.querySelector('#dlg > iframe').onclick = () => { event.stopPropagation(); }
      console.error(`iframe mode.`);
    } else {
...
function syntaxValidator() {
  let inp = document.querySelector("#synInput");
  inp.addEventListener("keydown", (k) => {
    if (k.key === "Enter") {
      if (urlArgs === "debug") { // debug mode on
        document.querySelector("#synInput").blur();
        document.body.insertAdjacentHTML("afterbegin", `
          <div id="dlg" onclick="{ document.querySelector('#dlg').remove(); }">
            <div onclick="{ event.stopPropagation(); }">
              ...
              <button onclick="{
                ...
                setTimeout(() => { syn(btoa(document.querySelector('#synInput').value)); }, 50);
              }">OK</button>
            </div>
          </div>
        `);
